...
async function seed() {
	...
	const TEACHER_EMAIL = "geldopc@gmail.com";
	...
	await db.collection("admins").doc(TEACHER_EMAIL).set(
		{
			status: "active",
			invitedBy: "system",
	...
	console.log(
		`Done! Seeded ${LESSON_ID} with ${phrasesData.length} phrases + teacher ${TEACHER_EMAIL}`
	);
}
